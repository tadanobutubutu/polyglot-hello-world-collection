# P4

113 entries.

##### 'Python' is not recognized
```text
|H|e|l|l|o|,| |w|o|r|l|d|!
```
[Source File](../libraries/p/P-4/'Python'%20is%20not%20recognized)

##### *python
```text
form[print[string[Hello, world\n]]];#H
```
[Source File](../libraries/p/P-4/*python)

##### P0pCrn
```text
_P    _0    _p    _!
@o    #o    $o    st
p_    p_    p_    r_
Th    ms    P0    op
is    g_    pC 
is    op    rn
wh    P0    =_
at    pC    He
co    rn    ll
mm          o_
en          wo
ts          rl
lo          d!
ok
li 
ke 
in
P0
pC
rn
!

Outputs "Hello, world!"
```
[Source File](../libraries/p/P-4/P0pCrn)

##### PPL
```text
;Hello World in PPL (PCBoard Programming Language)
PRINTLN "Hello, World!"
```
[Source File](../libraries/p/P-4/PPL)

##### PQN-PROC
```text
PQN
C Hello World in PQN/PROC
O Hello World
```
[Source File](../libraries/p/P-4/PQN-PROC)

##### PRAAT
```text
# Hello World in praat (www.praat.org)
echo Hello World!
```
[Source File](../libraries/p/P-4/PRAAT)

##### Prelude
```text
...8+.!v...4+.1+!..v!..vvv...4+.!vvv.....9+.3-!###!..v6-!..vv..v
9+(1-)^#7+(1-)..^7+!^3+!##8+(1-)^###9+1+(1-)..^^^^^3+!..^8-!#1+!
```
[Source File](../libraries/p/P-4/Prelude)

##### Preparation
```text
1234
2232
3456
1475
1794
2365
1764
3655
4026
4480
603
550
624
25
66
135
108
143
12
16
45
28 <--\
48    |
7     |
6     |
9     |
28 ---/
```
[Source File](../libraries/p/P-4/Preparation)

##### Preserve Line Numbers
```text
1 Blah Blah Blah, no matter what this is.
```
[Source File](../libraries/p/P-4/Preserve%20Line%20Numbers)

##### PRG
```text
VAR CHR CHL
VAR CHR CHO
SET CHL SUB POW TEN TWO TEN
SET CHO SUB POW ADD TEN ONE TWO TEN
PUT ARR 
    MUL ADD TEN TWO SIX
    ADD POW TEN TWO ONE
    CHL
    CHL
    CHO
    ADD MUL ADD SIX ONE SIX TWO
    POW TWO SUB SIX ONE
    ADD ADD CHL TEN ONE
    CHO
    SUB MUL ADD TEN TWO TEN SIX
    CHL
    POW TEN TWO
    ADD POW TWO SUB SIX ONE ONE
END
```
[Source File](../libraries/p/P-4/PRG)

##### Pride
```text
***!*!**!*!.
--------
!*!*!*!**!*.
--------
**!**!*!*..
--------
!****!*!**!.
--------
*****!*!*.
--------
!***!*!*!*!*!.
--------
!****!*!**!.
--------
*!*!**!***!.
--------
**!**!*!*.
--------
**!*!**!**!.
```
[Source File](../libraries/p/P-4/Pride)

##### PrimeFuck
```text
pd
```
[Source File](../libraries/p/P-4/PrimeFuck)

##### Primes
```text
   32 32 32 32 32 32 32 32 32 32 128 16 32 32 32 32 32 32 32 16 32 32 32 32 32 32 32 32 32 32 16 32 32 32 32 32 32 32 32 32 32 32 16 32 32 32 16 32 32 32 32 32 32 32 32 32 16 32 32 32 32 32 32 32 32 32 32 8 8 8 
   8 8 8 64 256 16 32 32 512 16 32 512 16 64 64 512 512 32 32 32 512 16 32 32 512 16 64 64 64 512 8 8 512 32 32 32 512 64 64 64 64 64 64 512 16 16 16 512 8 8 32 512 0
```
[Source File](../libraries/p/P-4/Primes)

##### PRINT
```text
import java.util.Scanner;

class HelloWorld {
    public static void main(String[] args) {
       System.out.println("What is your source code?");
       Scanner scanner = new Scanner(System.in);
        char[] cmds = scanner.nextLine().toCharArray();
        int index = 0;
        while (index < cmds.length) {
            char cmd = cmds[index];
            switch (cmd) {
                case 'p':
                    System.out.print("\nprint"); 
                    break;
                case 'w':
                    System.out.print("write"); 
                    break;
                case 'r':
                    index = -1;
                    break;
            }
            index++;
        }
    }
}
```
[Source File](../libraries/p/P-4/PRINT)

##### Print Function Deluxe
```text
pHelplo,p Woprldp!
```
[Source File](../libraries/p/P-4/Print%20Function%20Deluxe)

##### PRINTASKSWITCHINPUTCASEXGOTOACASEYGOTOBELSEGOTOC
```text
PRINT "HELLO WORLD!" ASK SWITCH INPUT ELSE GOTO 0
```
[Source File](../libraries/p/P-4/PRINTASKSWITCHINPUTCASEXGOTOACASEYGOTOBELSEGOTOC)

##### Prints the name of this language
```text
Prints the name of this language
```
[Source File](../libraries/p/P-4/Prints%20the%20name%20of%20this%20language)

##### Printscript
```text
print "Hello, world!"
```
[Source File](../libraries/p/P-4/Printscript)

##### Printscript 13
```text
set a !
set b !
add a b
pint a
```
[Source File](../libraries/p/P-4/Printscript%2013)

##### Printscript 5
```text
-This is a one-line comment.
-print "asdf"
print "The previous 2 lines are skipped."
```
[Source File](../libraries/p/P-4/Printscript%205)

##### Printscript 9
```text
set a 1
while a
	print "a"
end
```
[Source File](../libraries/p/P-4/Printscript%209)

##### Prisma
```text
datasource db {
  provider = "sqlite"
  url      = "file:./hello.db"
}
// Hello World schema stub
```
[Source File](../libraries/p/P-4/Prisma)

##### PRNGP2
```text
494;328;6;53;291;49;104;131;30;116;116;5;140;139;205;5;328;116;95;31;104;32
```
[Source File](../libraries/p/P-4/PRNGP2)

##### Probie
```text
↓P...........<
.HELLO WORLD!.
```
[Source File](../libraries/p/P-4/Probie)

##### ProC
```text
/* Hello World in Pro*C, the Oracle's embedded SQL environment */

#include <stdio.h>

EXEC SQL INCLUDE SQLCA;

int main() {
   char hello[15];
   char *user = "the_user";
   char *password = "the_password";
   char *sid = "the_sid";

   EXEC SQL CONNECT :user IDENTIFIED BY :password USING :sid;

   EXEC SQL
      SELECT 'Hello World' INTO :hello
      FROM DUAL;

   printf("%s\n", hello);

   EXEC SQL COMMIT RELEASE;
 
   return 0;
}
```
[Source File](../libraries/p/P-4/ProC)

##### Procedural Footnote Language
```text
[1]
[PFL1.0]
[1] Hello, world!
[PFLEND]
```
[Source File](../libraries/p/P-4/Procedural%20Footnote%20Language)

##### Processing
```javascript
void setup() {
  println("Hello World");
}
```
[Source File](../libraries/p/P-4/Processing.js)

##### Processor
```text
! => Logical NOT of DATA, writes to DATA
& => Logical AND between selected register and DATA, writes to DATA
^ => Logical XOR between selected register and DATA, writes to DATA
, => Copy DATA to a (selects a)
$ => Pops a to DATA (doesn't erase a)
. => Copy DATA to b (selects b)
* => Pops b to DATA (does erase b)
/ => Copy DATA to c (selects c)
? => Pops c to DATA (does erase c)
; => Copy DATA to d (selects d)
: => Pops d to DATA (does erase d)
@ => Copy DATA to e (selects e)
# => Pops e to DATA (does erase e)
[ => Copy DATA to f (selects f)
{ => Pops f to DATA (does erase f)
] => Copy DATA to g (selects g)
} => Pops g to DATA (does erase g)
\ => Copy DATA to h (selects h)
| => Pops h to DATA (does erase h)
> => Moves DATA to the RIGHT
< => Moves DATA to the LEFT
+ => Increment DATA
- => Decrement DATA
~ => Print DATA to STDOUT
( => Target for ')'
) => If DATA is zero, jump to the most recent '('. Otherwise, do nothing.
% => bitshift DATA left
_ => bitshift DATA right, mod 256
` => nop
```
[Source File](../libraries/p/P-4/Processor.txt)

##### Procfile
```text
web: echo Hello World
```
[Source File](../libraries/p/P-4/Procfile)

##### ProDOS
```text
printline Hello world!
```
[Source File](../libraries/p/P-4/ProDOS)

##### Profan
```text
' Hello World in Profan (http://www.profan.de/)

cls
print "Hello World!"
waitkey
```
[Source File](../libraries/p/P-4/Profan)

##### ProgFk
```text
14 11 14 11 11 14 14 11 14 11 41 14 14 11 41 41 11 14 14 11 41 41 11 14 14 11 41 41 41 41 11 41 14 14 11 11
14 11 11 11 14 14 14 11 41 41 41 14 14 11 41 41 41 41 14 14 14 11 14 11 14 14 11 41 41 11 14 14 11 14 11 11
14 11 11 14 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22
22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 22 27 11 11 11 11 71 11 11 11 17 11 11 11 11 71 11 11
11 17 11 11 11 11 71 11 11 11 17 11 11 11 11 71 11 11 11 17 11 11 11 11 71 11 11 11 17 11 11 11 11 71 11 11
11 17 11 11 11 11 F0
```
[Source File](../libraries/p/P-4/ProgFk)

##### Programming Language
```text
print(Hello world!)
```
[Source File](../libraries/p/P-4/Programming%20Language)

##### Programming Language for Business
```text
DISPLAY 'Hello World'
```
[Source File](../libraries/p/P-4/Programming%20Language%20for%20Business)

##### Prograph
```text
// Prograph is visual dataflow; conceptual: print "Hello World"
```
[Source File](../libraries/p/P-4/Prograph)

##### Progress
```text
/* Hello World in Progress */

message "Hello World" view-as alert-box.
```
[Source File](../libraries/p/P-4/Progress)

##### Progress ABL
```text
MESSAGE "Hello World" VIEW-AS ALERT-BOX.
```
[Source File](../libraries/p/P-4/Progress%20ABL)

##### ProgressBASIC
```text
10 PRINT "Hello world"
```
[Source File](../libraries/p/P-4/ProgressBASIC)

##### PROIV
```text
DISPLAY "Hello World"
```
[Source File](../libraries/p/P-4/PROIV)

##### Project Verona
```text
print("Hello World"); // Verona research language
```
[Source File](../libraries/p/P-4/Project%20Verona)

##### PROLAN-M
```text
(<original string>,<replacement string>)
```
[Source File](../libraries/p/P-4/PROLAN-M)

##### Prolog
```text
% Hello World in Prolog

hello :- display('Hello World!') , nl .
```
[Source File](../libraries/p/P-4/Prolog)

##### Prolog  _Ciao
```text
main :-
     write('Hello, World!').
```
[Source File](../libraries/p/P-4/Prolog%20%20_Ciao)

##### Prolog  _SWI
```text
?-write('Hello, World!').
```
[Source File](../libraries/p/P-4/Prolog%20%20_SWI)

##### PROMAL
```text
program hello
include library
begin
output "Hello world!"
end
```
[Source File](../libraries/p/P-4/PROMAL)

##### Promela
```text
active proctype hello() {
  printf("Hello World\n")
}
```
[Source File](../libraries/p/P-4/Promela)

##### Propeller Spin
```text
PUB main
  ' Hello World on Propeller (serial out depends on hardware)
```
[Source File](../libraries/p/P-4/Propeller%20Spin)

##### PROSE
```text
PRINT "Hello World"
```
[Source File](../libraries/p/P-4/PROSE)

##### PROTEL
```text
DISPLAY "Hello World"
```
[Source File](../libraries/p/P-4/PROTEL)

##### Protobuf
```text
syntax = "proto3";
message Hello { string world = 1; }
```
[Source File](../libraries/p/P-4/Protobuf)

##### Protogen
```text
02 00 00 00
41 48 00 00
2E 00 00 00
41 65 00 00 
2E 00 00 00 
41 6C 00 00
2E 00 00 00
2E 00 00 00
41 6F 00 00
2E 00 00 00
41 2C 00 00
2E 00 00 00
41 20 00 00
2E 00 00 00
41 57 00 00
2E 00 00 00
41 6F 00 00
2E 00 00 00
41 72 00 00
2E 00 00 00
41 6C 00 00
2E 00 00 00
41 64 00 00
2E 00 00 00
41 21 00 00
2E 00 00 00
00 00 00 00
```
[Source File](../libraries/p/P-4/Protogen)

##### PRSCNT
```text
SET A, B, C, D... - Changes the value of variables pointed by A, B, C, D...
INC A, B, C, D... - Increments the value of variables pointed by A, B, C, D...
NEW A, B, C, D... - Create new variables to be pointed by A, B, C, D...
DEL A, B, C, D... - Delete variables pointed by A, B, C, D...
```
[Source File](../libraries/p/P-4/PRSCNT)

##### Pseudocode  _College Board
```text
DISPLAY (expr)
INPUT ()
RANDOM (a, b)
```
[Source File](../libraries/p/P-4/Pseudocode%20%20_College%20Board)

##### PSL
```text
// Property Specification Language stub: Hello World
```
[Source File](../libraries/p/P-4/PSL)

##### PSQL
```text
 EXECUTE BLOCK
   RETURNS(S VARCHAR(40))
 AS
 BEGIN
   S = 'Hello world!';
   SUSPEND;
 END
```
[Source File](../libraries/p/P-4/PSQL)

##### PTotE
```text
Eu U Es Es No V Pt No Db Es Pa Cr
```
[Source File](../libraries/p/P-4/PTotE)

##### PUBERTY
```text
It is June 12, 2018, 03:02:04 PM. Izu is in his bed, bored. His secret kinks are vore, fatfurs, inflation, growth and kitsunes. 
Then he spots fatfurs. Soon the following sounds become audible. 

fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap yes fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap yes yeah yeah yeah yes yes fap fap fap yes yEAH YeaH 
fAtfURs,hNNg yEAH fap fap inflation,hnng yeah fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap vore,hnng 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap yeah fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap vore,hnng fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap yeah ugh fap fap fap fap fap
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fatfurs,hnng yeah fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap fap 
fap fap fap fap fap fap fap fap fap vore,hnng yeah squirt ugh 
growth,hnng kitsunes,fuck yes sigh
```
[Source File](../libraries/p/P-4/PUBERTY)

##### Pulumi
```text
import * as pulumi from "@pulumi/pulumi";
export const hello = "Hello World";
```
[Source File](../libraries/p/P-4/Pulumi)

##### Punc
```text
LISP:(a b (c) d) 
Punc:The contents of anonymous is:a, b, c, and d.
```
[Source File](../libraries/p/P-4/Punc)

##### Punctree
```text
T ::= ‘0’          -- a leaf (an empty tree)
   |  ‘2’ T T      -- branching to two subtrees

C ::= ‘_’          -- a hole (an empty context)
   |  ‘2’ C T      -- branching to two subtrees: one with a hole, the other, whole
   |  ‘2’ T C      -- the same with arguments swapped

Θ ::= T | C        -- in case we don’t care or know
```
[Source File](../libraries/p/P-4/Punctree)

##### Punyforth
```text
\ Hello world in Punyforth

println: "Hello world!"
```
[Source File](../libraries/p/P-4/Punyforth)

##### Puppet
```text
notify { "Hello World": }
```
[Source File](../libraries/p/P-4/Puppet)

##### PUPPY
```text
BARKWOOFBARKWOOFBARKWOOFBARKWOOFBARKWOOFBARKWOOFBARKWOOFBARKWOOFBARKWOOF
```
[Source File](../libraries/p/P-4/PUPPY)

##### Pure
```text
using system;

puts "Hello world!\n" ;
```
[Source File](../libraries/p/P-4/Pure)

##### Pure BF
```text
   (Tape, World) -> (Tape, World)
```
[Source File](../libraries/p/P-4/Pure%20BF)

##### PureBasic
```text
OpenConsole()
PrintN("Hello world!")
Input() ; Wait for enter
```
[Source File](../libraries/p/P-4/PureBasic)

##### PureScript
```text
module Main where
import Effect.Console (log)
main = log "Hello World"
```
[Source File](../libraries/p/P-4/PureScript)

##### PureScript language
```text
module Main where
import Effect.Console (log)
main = log "Hello World"
```
[Source File](../libraries/p/P-4/PureScript%20language)

##### PureStack
```text
"Hello, World!"
!~
```
[Source File](../libraries/p/P-4/PureStack)

##### Purl
```text
K1 P4 K1 EOR
K2 P2 K1 P2 EOR
K2 P1 K2 P2 EOR
K3 P2 K1 P1 EOR
K2 P1 K4 EOR
K3 P1 K3 EOR
K1 P5 EOR
K1 P1 K2 P2 EOR
K2 P1 K4 EOR
K2 P1 K2 P2 EOR
K2 P1 K2 P2 EOR
K2 P2 K1 P1 K1 EOR
K1 P2 K1 P3 EOR
DROP DROP DROP DROP DROP DROP DROP DROP DROP DROP DROP DROP DROP
```
[Source File](../libraries/p/P-4/Purl)

##### Purple
```text
aA1aa1oA0bi0b0bb1bbbAiB0 Purple is the awesomest, so why haven't you tried it yet?
!dlroW ,olleH
```
[Source File](../libraries/p/P-4/Purple)

##### PUSH
```text
H   e    l    l    o    ,  " "  W    o    r    l    d   !   \n
72, 101, 108, 108, 111, 44, 32, 87, 111, 114, 108, 100, 33, 10
```
[Source File](../libraries/p/P-4/PUSH)

##### PushDupAddSub
```text
1 - push 1
+ - pop a and b, push a+b
- - pop a and b, push b-a
/ - pop a and b, push b/a, pushing quotient followed by remainder
: - duplicate
[...] - while 0
```
[Source File](../libraries/p/P-4/PushDupAddSub)

##### PVS
```text
%|- Hello World PVS stub
```
[Source File](../libraries/p/P-4/PVS)

##### Pxem
```text
Hello.p	Prints "Hello"
Aa.-	Makes ' '
Am.-	Makes ','
.p	Prints ", "
world.p	Prints "world"
ak.-	Makes '\n'
Ab.-	Makes '!'
.p	Prints "!\n"
xe
```
[Source File](../libraries/p/P-4/Pxem)

##### Pxem

*ZIP アーカイブ型プログラム / 134 bytes*

**抽出ソース / 要約**

```text
// archive members:
//   Hello World.p.pxe  (2 bytes)

// --- Hello World.p.pxe (content 2 bytes) ---
# filename is significant for some esolangs (e.g. Pxem)
 

```

[Source File](../libraries/p/P-4/Pxem.zip)

##### PyChr
```text
6>^5165_~h1'>>6(I()5<11'h?~|8
```
[Source File](../libraries/p/P-4/PyChr)

##### PyFuck  _kuangkzh
```text
6>^5165_~h1'>>6(I()5<11'h?~|8
```
[Source File](../libraries/p/P-4/PyFuck%20%20_kuangkzh)

##### PyFuck  _shirAko
```text
6>^5165_~h1'>>6(I()5<11'h?~|8
```
[Source File](../libraries/p/P-4/PyFuck%20%20_shirAko)

##### Pyfun
```text
[[[[]<[[]]]+[[]]]+[[[[]<[[]]][[]<[]]+[[]<[[]]][[]<[]]]+[a[[[[]<[]]+[[]<[[]]]][[]<[]][[[]<[]][[]<[]]<a[[]<[]]]]]]+[[[]<[[]]]+[[]]]][[]<[]][a[[]<[]]]
```
[Source File](../libraries/p/P-4/Pyfun)

##### Pyhton  _sic
```text
6>^5165_~h1'>>6(I()5<11'h?~|8
```
[Source File](../libraries/p/P-4/Pyhton%20%20_sic)

##### Pyline
```text
print("hello world")
```
[Source File](../libraries/p/P-4/Pyline)

##### Pyline Classic
```text
print("hello world")
```
[Source File](../libraries/p/P-4/Pyline%20Classic)

##### Pyodide
```text
print("Hello World")
```
[Source File](../libraries/p/P-4/Pyodide)

##### PyPy
```text
print("Hello World")
```
[Source File](../libraries/p/P-4/PyPy)

##### Pyret
```text
print("Hello, World!\n")
```
[Source File](../libraries/p/P-4/Pyret)

##### Pyrex
```text
print "Hello World"
```
[Source File](../libraries/p/P-4/Pyrex)

##### PYSP
```text
USE_SAFE_SENTINEL
```
[Source File](../libraries/p/P-4/PYSP)

##### Pyston
```text
print("Hello World")
```
[Source File](../libraries/p/P-4/Pyston)

##### PyText?!
```text
import os
s = open(input("Run Script: "), 'r').read()
s = s.replace('.>', 'a')
s = s.replace('..>', 'b')
s = s.replace('...>', 'c')
s = s.replace('!.>', 'd')
s = s.replace('!..>', 'e')
s = s.replace('!...>', 'f')
s = s.replace('!!.>', 'g')
s = s.replace('!!..>', 'h')
s = s.replace('!!...>', 'i')
s = s.replace('?.>', 'j')
s = s.replace('?..>', 'k')
s = s.replace('?...>', 'l')
s = s.replace('??.>', 'm')
s = s.replace('??..>', 'n')
s = s.replace('??...>', 'o')
s = s.replace(';.>', 'p')
s = s.replace(';..>', 'q')
s = s.replace(';...>', 'r')
s = s.replace(';;.>', 's')
s = s.replace(';;..>', 't')
s = s.replace(';;...>', 'u')
s = s.replace(':.>', 'v')
s = s.replace(':..>', 'w')
s = s.replace(':...>', 'x')
s = s.replace('::.>', 'y')
s = s.replace('::..>', 'z')
s = s.replace('::...>', '1')
print("Result is ", s)
```
[Source File](../libraries/p/P-4/PyText?!)

##### Pyth
```text
"Hello, World!
```
[Source File](../libraries/p/P-4/Pyth)

##### PythBrSc
```text
print("Hello, world!");
```
[Source File](../libraries/p/P-4/PythBrSc)

##### Python 2
```text
# Hello world in Python 2

print "Hello World"
```
[Source File](../libraries/p/P-4/Python%202)

##### Python 2  _Cython
```text
print "Hello, World!"
```
[Source File](../libraries/p/P-4/Python%202%20%20_Cython)

##### Python 2  _IronPython
```text
print "Hello, World!"
```
[Source File](../libraries/p/P-4/Python%202%20%20_IronPython)

##### Python 2  _Jython
```text
print "Hello, World!"
```
[Source File](../libraries/p/P-4/Python%202%20%20_Jython)

##### Python 2  _PyPy
```text
print "Hello, World!"
```
[Source File](../libraries/p/P-4/Python%202%20%20_PyPy)

##### Python 3
```text
# Hello world in Python 3 (aka Python 3000)

print("Hello World")
```
[Source File](../libraries/p/P-4/Python%203)

##### Python 3  _Cython
```text
print("Hello, World!")
```
[Source File](../libraries/p/P-4/Python%203%20%20_Cython)

##### Python 3  _PyPy
```text
print("Hello, World!")
```
[Source File](../libraries/p/P-4/Python%203%20%20_PyPy)

##### Python 3  _Stackless
```text
print("Hello, World!")
```
[Source File](../libraries/p/P-4/Python%203%20%20_Stackless)

##### Python 3
```text
print("Hello, World!")
```
[Source File](../libraries/p/P-4/Python%203.8%20%20_pre-release)

##### Python but annoying
```text
x = 1 / 0
```
[Source File](../libraries/p/P-4/Python%20but%20annoying)

##### Python But Bad
```text
import os,sys
with open(sys.argv[1],"r",encoding="utf-8") as f:
    c=f.read()
try:
    exec(c)
except:
    os.remove(sys.argv[1])
```
[Source File](../libraries/p/P-4/Python%20But%20Bad)

##### Python but even worse
```text
print("Hello World!")
1/0
```
[Source File](../libraries/p/P-4/Python%20but%20even%20worse)

##### Python But WORST
```text
print('Hello World!')
1/0 # division by zero will cause an error
```
[Source File](../libraries/p/P-4/Python%20But%20WORST)

##### Python console
```text
>>> print("Hello World")
```
[Source File](../libraries/p/P-4/Python%20console)

##### Python for S60
```text
print "Hello World"
```
[Source File](../libraries/p/P-4/Python%20for%20S60)

##### Python is Magic
```text
__name__.__class__.__name__.__len__().__sub__(__name__.__class__.__name__.__len__())
```
[Source File](../libraries/p/P-4/Python%20is%20Magic)

##### Python traceback
```text
Hello World
```
[Source File](../libraries/p/P-4/Python%20traceback)

##### Python, but it received the worst translation
```text
version("Hello, world!")
```
[Source File](../libraries/p/P-4/Python,%20but%20it%20received%20the%20worst%20translation)

##### Pythonista
```text
print("Hello World")
```
[Source File](../libraries/p/P-4/Pythonista)

##### PZAB
```text
<<<Z<<<Z;
>Z>>!Z!>Z>>Z;
!Z!<<Z<Z;;
>>Z>Z;
!Z<Z<<<<<Z;
>>>Z>Z!;
Z>Z>Z<<<<Z<<Z;
!>>Z!>Z;
!Z>Z>>Z!<<<<Z;
!Z!>Z>Z>!Z;
<<Z;
<<<Z>>>>Z!>>Z;
```
[Source File](../libraries/p/P-4/PZAB)

